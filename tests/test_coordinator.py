...
from custom_components.lamax_connect.lamax import Device, LamaxAuthError, LamaxError
...
async def test_entities_go_unavailable_on_update_failure(
    ...
    mock_client.async_get_devices_with_location.side_effect = LamaxError(557, "boom")
    await _advance(hass, freezer)
    assert hass.states.get("sensor.junior_battery").state == STATE_UNAVAILABLE

    mock_client.async_get_devices_with_location.side_effect = None
    await _advance(hass, freezer)
    assert hass.states.get("sensor.junior_battery").state == "100"
...
async def test_auth_failure_during_refresh_triggers_reauth(
    ...
    mock_client.async_get_devices_with_location.side_effect = LamaxAuthError(25, "gone")
    await _advance(hass, freezer)
...
async def test_watch_added_later_creates_entities(
    ...
    mock_client.async_get_devices_with_location.return_value = {
        device.imei: (device, location),
        second.imei: (second, location),
    }
    await _advance(hass, freezer)
